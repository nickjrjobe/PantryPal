...
public class RecipeListFactoryTest {
  ...
  public void testJSONObjectConstructor() {
    ...
    assertEquals(
        "Add peanut butter and jelly to sandwich bread.",
        recipe.getDescription(),
        "The description of the recipe should match");
  }
  ...
  public void testJSONArrayConstructor() {
    ...
    assertEquals(
        "Recipe 1 title", recipe1.getTitle(), "The title of the first recipe should match");
    assertEquals(
        "breakfast", recipe1.getMealType(), "The meal type of the first recipe should match");
    assertEquals(
        "Recipe 1 description",
        recipe1.getDescription(),
        "The description of the first recipe should match");

    // Check the details of the second recipe in the list
    Recipe recipe2 = recipeList.get(1);
    assertEquals(
        "Recipe 2 title", recipe2.getTitle(), "The title of the second recipe should match");
    ...
    assertEquals(
        ...
        "The description of the second recipe should match");
  }
  ...
  public void testBuildJSON() {
    ...
    assertEquals(
        rlf.buildList().size(),
        recipesArray.length(),
        "The JSONArray should have the same number of elements as the recipe list");
    ...
    for (int i = 0; i < recipesArray.length(); i++) {
      ...
      assertEquals(
          ...
          recipeJSON.toString(),
          "The JSON object should match the expected recipe JSON");
    }
  }
  ...
  public void testSort() {
    ...
    for (int i = 0; i < originalList.size(); i++) {
      assertEquals(
          originalList.get(i).getTitle(),
          sortedRecipes.get(i).getTitle(),
          "List order should remain unchanged");
    }
  }
}
